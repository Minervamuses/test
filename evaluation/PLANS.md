...
- **2026-09-19 phase-04 實測（沙箱 CPU，取代上面那條推估）：** 單張 4056×3040 端到端 **30.3 秒**、程序峰值 RSS **4287 MiB**。拆解：SR 推論約 12 秒、SSIM 兩次共約 8.7 秒、LPIPS 兩次約 6 秒、退化與 bicubic 各約 2.3 秒。十分鐘授權內在 CPU 上約 **20 張**。
  - 首次量到的是 85.5 秒／15.4 GiB，肇因為全尺寸 SSIM 的原始實作；改為逐通道可分離高斯後成為上列數字，值僅差 `2.665e-15`。原委見 [context/phase-04-context.md](context/phase-04-context.md)。
  - **裝置歸屬（重要）：** PSNR 與 SSIM 的輸入來自 Pillow，**永遠在 CPU 以 float64 計算**，不隨 SR 裝置改變，因此跨裝置可精確重現；只有 SR 線與 LPIPS 跟隨裝置。報告標頭已分欄記載。
  - 磁碟：每張 run 產物約 **53 MB**（hr／lr／bicubic／sr 四個 PNG）。
