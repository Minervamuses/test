...
## 2026-09-19 02:40 (CST) — Phase 05: preflight

- **狀態：** `Not started` → `In progress`
- **授權範圍：** [phases/phase-05-acceptance-and-docs.md](phases/phase-05-acceptance-and-docs.md)。依賴 phase-04 已 `Complete`。
- **本階段範圍（複述）：** 產生並煙霧測試 `evaluation/gpu_checks/run_on_user_shell.sh`、一次真實小樣本執行、四種影像的目視檢查、讀一次 `report.md`、估算全量成本、寫 `evaluation/README.md`、對齊 `README.md` 的範圍敘述、逐條核對成功條件。
- **非目標（複述）：** **不跑全量 738 張**；不因數字不如預期調整退化流程、度量約定或模型；不做模型比較或畫質優化；**不改 `AGENTS.md`**。
- **停止條件（複述）：** 全量執行或預估超過十分鐘；需要改 `AGENTS.md`；小樣本結果與某項成功條件矛盾且原因不明。
- **驗證（實際觀察）：**
  - 磁碟：`df -h .` → 可用 **836 GB**。phase-04 實測每張產物約 53 MB，5 張約 265 MB，空間充裕。
  - 目前 `evaluation/runs/` 共四個 run、合計 **445 MB**。
  - **取樣代表性：** `input/` 是同一次飛行、連續 738 張，時間跨度 `11:57:59`（0001）到 `13:14:52`（0362）。依序取前 5 張只會覆蓋開頭一分鐘，代表性差。改用 `--seed 20260919` 隨機取樣，實得 `0101`（12:02）、`0143`（12:04）、`0367`（12:14）、`0273`（13:10）、`0295`（13:11），涵蓋飛行前段與後段。**即便如此仍是同一次飛行、同一片場景，不能外推到其他場景。**
  - **交叉核對機會（本次新識別）：** 依 `context/phase-04-context.md`，bicubic 線的放大是純 Pillow CPU、PSNR 與 SSIM 也恆在 CPU float64。因此**同一批影像在 CPU 批次與 GPU 批次之間，bicubic 線的 PSNR／SSIM 應當逐位數相同**。交接腳本將沿用同一個 seed 與 limit，讓這項比對可以做，成為「跨裝置只有 SR 與 LPIPS 會變」這個說法的直接證據。
- **限制：** 本筆為唯讀 preflight。交接腳本尚未撰寫，小樣本尚未執行。
- **阻塞：** 無。
