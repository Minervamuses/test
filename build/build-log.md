...
### 2026-09-17T21:58:38+08:00 — Phase 04 完整 4K 批次與視覺驗收

- 前一步 `4c2d928` 提交兩模型真實小例，沒有再改 production。實際 WSL 專案根目錄命令 `.venv/bin/python -u test-data/phase-04-tiling-20260917/validate_full.py`，驗證腳本 exit0。唯一真正 CLI 子程序（timeout300秒）為 `.venv/bin/python -m drone_sr --input '/home/minervamuses/drone-image-analysis/test-data/phase-04-tiling-20260917/full batch/input' --output '/home/minervamuses/drone-image-analysis/test-data/phase-04-tiling-20260917/full batch/output'`。
- 正式 Compact／FP32／cuda:0，一次模型載入、3張序列批次：a_full.png 完整3840×2160，b_broken.PNG 故意損壞，c_small.png 真實32×28；後者是同一 frame box(3808,2132,3840,2160)。CLI **Processed2／Failed1／exit1**為預期，壞圖後小圖成功。整個子程序含啟動／讀圖／模型／PNG／壞圖 **37.955755秒**；resource.getrusage(RUSAGE_CHILDREN).ru_maxrss **5,704,076 KiB（約5.44GiB）**，為此唯一CLI子程序峰值RSS，不是GPU VRAM。沒有額外跑第二次4K量測。
- a_full.png 為既有frame的完整副本，未縮圖，input SHA `0ca08a179e42c9917d1bab23ebc354d1a8791833b27e81a1dc2a58b662ad9f31`。40個core512、各側halo32自動拼接，输出 **15360×8640 RGB PNG**、**117,800,469 bytes**，SHA **`bce5bfc0b9b347f06982e13f56c49ee6cc43ca910f1bf0c558887e4d613d7822`**。Pillow Image.open＋load 完整解碼成功、尺寸嚴格4×；沒有mock／跳過模型或用縮圖代替大型輸入。
- c_small.png input SHA `7f31e431264556d50f7d3071566f63c280673b8fb2603bc0bf62d1e4b15e70e5`；output **128×112**、11,631bytes，SHA `a873765fd578428a1680ed4b07e7ae0284a072385fc81ca4ef286342f5b9c867`，已開啟內容正常。壞圖bytes為not a valid image、SHA `c4f45c8c1609877246f57c4a90ad8477e10096b6cdbee40a9236e38e2a6dd291`，沒有output，未冒充成功。
- 原frame、所有input、model.pth/Compact、既有phase-01 PNG與無關untouched.txt前後hash相同；驗收目錄中原1×1紅色a_full.png（SHA b1ff9c8e…6640）成功替換為本次完整PNG。完整來源／前後hash／argv／elapsed／RSS／輸出／視覺裁切記於 `test-data/phase-04-tiling-20260917/full batch/validation.json`，console為cli.txt，重現紀錄為validate_full.py；不將此一次性腳本當CLI或benchmark。
- 檢視工具限制：直接 view_image 完整117.8MB PNG，base64 IPC frame為157,067,464bytes，超過67,108,864限制，工具失敗；**不是PNG解碼或推論失敗**。已改以完整PNG解碼後的1920×1080全圖縮覽（full-preview.png，SHA `e04785d97e0a1f6d69a7303b9fb8fc68f6242e4d1bece960eb0656d1038f5fd1`）與原尺寸像素裁切進行人工檢視，未改完整輸出或以縮圖掩蓋大型處理。
- 已開啟全圖縮覽與5個512×512原尺寸裁切：bright_seam(1792,1792,2304,2304)、middle_seam(7936,3840,8448,4352)、right_seam(14848,3840,15360,4352)、bottom_seam(7936,8128,8448,8640)、bottom_right(14848,8128,15360,8640)。各檔同名PNG，所有crop box與SHA在validation.json；前兩個跨核心接點，後三個到達真正最右／最下／右下像素。波紋、反光與海面色彩連續，沒有可見直線拼縫、空白條、重影或裁切缺失；只涵蓋此海面樣本，沒有道路／屋頂／鯨魚辨識驗證或高解析度真值。
