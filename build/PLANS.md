...
2026-09-17 使用者選定 `realesr-general-x4v3.pth`，授權依本機 VRAM 判斷採用並開始驗證。允許取得這一個官方 Compact checkpoint，以及從已授權的本機 WhaleDrone MP4 抽取小裁切副本，恢復 phase-01 真實驗收；原先等待使用者交付材料的描述，對此指定素材與權重改按本次授權處理。其他資料集檔案、第二模型權重與額外依賴不因此取得下載／安裝授權。沿用每步 commit 與既有階段依賴；4K 全圖／分塊不能由小裁切結果推定通過。

2026-09-17 使用者在 SwinIR-M real-world 4× 討論後指示「用512的，繼續」，授權取得官方 `003_realSR_BSRGAN_DFO_s64w8_SwinIR-M_x4_GAN.pth` 並以既有 512×512 真實裁切恢復 phase-03 驗證；這取代上段對此第二 checkpoint 的未授權狀態，不包含其他權重或資料。下載測速顯示可能超過十分鐘，完整下載仍須按下節確認具體時間成本；推論採既有 FP32、batch 1，共用路徑，每個程序上限 120 秒。

2026-09-17 使用者補充「批准，計劃內的東西不用再問我，一律批准」：批准上述最多 30 分鐘／70 MB 單次 SwinIR 下載，以及本計劃四階段內必要實作、驗證、文件、資源工作與 commit；已批准的計劃內工作不再重複詢問。執行前仍記錄成本並依序驗證、保留原始資料、遵守具體失敗／停止界線；不擴張 GOALS 非目標或下載其他資料集檔案。
