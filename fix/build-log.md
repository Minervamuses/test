...
### 2026-09-18 12:47 CST — 更正（追加）：phase-02 的行號

12:24 CST 該筆寫的位置為 `image_io.py:12-22` 與 `:31-33`；以最終檔案核對，實際為 helper `_source_bits_per_sample()` 在 **`image_io.py:12-21`**、位深檢查在 **`image_io.py:32-34`**（`:30-31` 為說明註解）。守門順序與該筆所述一致，結論不變。

### 2026-09-18 13:05 CST — 計劃外：使用者要求重寫 README

- **狀態：** 三階段仍為 Complete，本筆不改變任何階段狀態。
- **授權依據：** 使用者於計劃完成後明確要求「重寫 README」。此舉超出 phase-03 的非目標（「不重寫 README 其他章節」），由使用者新指示授權，非自行擴大範圍。
- **實際變更：** `README.md` 整份重寫（commit `e49ee20`），結構改為：涵蓋範圍／環境／安裝／執行（讀寫契約與失敗處理各自集中）／分塊／權重／驗證狀態（已驗證 vs 尚未驗證）／重跑檢查。
- **對本計劃記錄的影響：** 12:44 CST 該筆所述的 README 修改位置（第 66 行契約句、新增 `### 讀圖正確性修正（2026-09-18）` 一節）已被此次重寫取代。**修正相關的敘述與證據全部保留**，只是改置於「讀寫契約」與「驗證狀態」兩節；phase-03 的驗收結論不變。
- **驗證：** 以數值／雜湊逐項比對新舊 README，僅兩項未沿用 —— 已被 33／33 取代的 25／25 測試計數，以及計劃期的「67.13 MB 權重測速估約 24 分鐘」敘述，兩者皆仍在 git 歷史。所有 markdown 連結目標實際存在（`requirements-wsl.txt`、兩個 `test-data/` 產物、`fix/GOALS.md`、`fix/build-log.md`）；原本指向已移除 `build/` 的兩個死連結改為 `git show af6989e~1:build/build-log.md`，該命令實測可取回內容。另修正兩處簡體字（未驗证、後续）。
- **限制：** 純文件變更，未改程式、測試或依賴；未因此重跑模型（依 `AGENTS.md`「純文件變更只需確認內容與差異」）。重寫沿用既有已驗證敘述，未新增任何未驗證宣稱。
- **阻礙：** 無。
- **下一步：** 無；計劃維持完成狀態。
- **證據位置：** commit `e49ee20`。
